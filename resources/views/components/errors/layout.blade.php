...
<body class="min-h-screen flex flex-col bg-page">
    <header class="flex justify-center pt-10 sm:pt-14">
        ...
        <a href="{{ url('/') }}" class="inline-flex items-center gap-2.5">
            ...
            <span class="font-display" style="font-weight:800; font-size:24px; letter-spacing:-0.03em; color:var(--text-strong);">Atend<span class="text-brand">ia</span></span>
        </a>
    </header>

    <main class="relative flex-1 flex flex-col items-center justify-center px-5 py-12 overflow-hidden">
        <div aria-hidden="true" class="absolute inset-0 pointer-events-none" style="background: radial-gradient(60% 45% at 50% 42%, color-mix(in srgb, var(--brand) 12%, transparent), transparent 70%);"></div>
        ...
        <div class="relative w-full" style="max-width:32rem;">
            ...
            <div class="relative text-center" style="background:var(--bubble-in); color:var(--bubble-in-text); border:1px solid var(--border-subtle); border-radius:20px 20px 20px 6px; box-shadow:var(--shadow-lg); padding:32px 28px;">
                <p class="font-mono text-brand" style="font-size:var(--text-6xl); font-weight:700; line-height:1;">{{ $code }}</p>

                <h1 class="font-display text-strong mt-4" style="font-weight:800; font-size:var(--text-3xl); letter-spacing:-0.02em;">
                    {{ $title }}
                </h1>

                <p class="text-body mt-3" style="font-size:var(--text-base); line-height:1.6;">
                    {{ $message }}
                </p>
            ...
            <div class="flex items-end gap-3 mt-6" style="padding-left:8px;">
                <svg aria-hidden="true" viewBox="0 0 64 118" style="width:56px; height:103px; transform:rotate(-8deg);">
    ...
    <footer class="pb-8 text-center px-5">
        <p class="text-subtle" style="font-size:var(--text-sm);">© {{ date('Y') }} Atendia · Tu negocio, atendido por IA.</p>
    </footer>
</body>
</html>
