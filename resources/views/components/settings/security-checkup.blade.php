...
@php
    ...
    $passwordChangedAt = UserDto::fromUser($user)->password_changed_at;
...
<x-ui.card class="bp-card">
    ...
    {{-- One column: it lives in the 320px rail of the settings page. --}}
    <div class="mt-4 flex flex-col gap-3">
        <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
                ...
                @unless ($emailOk)
                    <livewire:settings.send-verification />
                @endunless
            </div>
        </div>
        ...
        <div class="flex items-center gap-3">
            ...
            <div class="min-w-0">
                ...
                {{-- Google/GitHub-style: how long the password has been in use. --}}
                <p class="text-muted" style="font-size: var(--text-xs)">
                    {{
                        $passwordChangedAt
                            ? __('profile.checkup.password_changed', ['ago' => $passwordChangedAt->diffForHumans()])
                            : __('profile.checkup.password_since_signup', ['ago' => $user->created_at?->diffForHumans()])
                    }}
                </p>
            </div>
        </div>
