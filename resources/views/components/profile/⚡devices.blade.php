...
/**
 * "Connected devices" card of the account profile: every browser the account
...
<x-ui.card id="dispositivos" class="p-5 sm:p-6" x-data="profileDevices">
    <div class="flex flex-wrap items-center justify-between gap-3">
        <h2 class="text-strong font-display" style="font-size: var(--text-lg); font-weight: 700">
            {{ __('profile.devices.title') }}
        </h2>
        @if ($this->devices->count() > 1 && ! $confirmingAll)
            <x-ui.button variant="secondary" size="sm" icon="log-out" wire:click="$set('confirmingAll', true)">
                {{ __('profile.devices.revoke_all') }}
            </x-ui.button>
        @endif
    </div>
    <p class="text-muted mt-1" style="font-size: var(--text-sm)">{{ __('profile.devices.sub') }}</p>
