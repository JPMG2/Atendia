...
test('retrieval is scoped to a single company and never leaks another tenant', function (): void {
    ...
    $docA = KnowledgeDocument::factory()->for($companyA)->create();
    $docB = KnowledgeDocument::factory()->for($companyB)->create();

    // Company A: two chunks pointing in different directions.
    ...
    // Company B: a chunk IDENTICAL to the query — it must never appear in A's results.
    ...
    $results = app(KnowledgeRetriever::class)->retrieve('lo que sea', $companyA->id);
...
test('the context block cites the source document title', function (): void {
    ...
    $document = KnowledgeDocument::factory()->for($company)->create(['title' => 'Cómo conectar WhatsApp']);
    ...
    $context = app(KnowledgeRetriever::class)->context('cómo conecto whatsapp', $company->id);
