<?php

declare(strict_types=1);

namespace App\Dto;

use App\Interfaces\Catalog\FormData;
use App\Models\Business;

/**
 * State of the business forms — the TENANT ({@see Business}). Full row on
 * purpose: wizard steps and the future profile edit slices of the same
 * record, each form hydrating from it first, so a partial screen never
 * blanks out what another saved. `sector` and `activity` are not columns —
 * they gate the wizard and feed the activity sync — so no `toPayload()`.
 */
class BusinessDto implements FormData
{
    /**
     * Create a new class instance.
     */
    public function __construct(
        public string $name = '',
        public ?int $country_id = null,
        public ?int $province_id = null,
        public ?string $timezone = null,
        public string $billing_email = '',
        public ?string $whatsapp_number = null,
        public ?string $fallback_whatsapp_number = null,
        public ?string $whatsapp_instance = null,
        public ?string $whatsapp_connected_at = null,
        public string $handoff_level = 'balanced',
        public ?string $handoff_rules = null,
        public ?string $email = null,
        public ?string $web = null,
        public ?string $logo_path = null,
        public ?string $address = null,
        public ?string $city = null,
        public ?bool $has_premises = null,
        public ?string $description = null,
        public ?int $currency_id = null,
        public ?int $reference_currency_id = null,
        public ?int $tax_condition_id = null,
        public ?string $tax_id = null,
        public bool $is_active = true,
        public ?string $sector = null,
        public ?string $activity = null,
    ) {}

    /**
     * Prepare the object for Livewire.
     */
    public function toLivewire()
    {
        return $this->toArray();
    }

    /**
     * Recreate the object from Livewire data.
     */
    public static function fromLivewire($value)
    {
        return self::fromArray(is_array($value) ? $value : []);
    }

    public function toArray(): array
    {
        return [
            'name' => $this->name,
            'country_id' => $this->country_id,
            'province_id' => $this->province_id,
            'timezone' => $this->timezone,
            'billing_email' => $this->billing_email,
            'whatsapp_number' => $this->whatsapp_number,
            'fallback_whatsapp_number' => $this->fallback_whatsapp_number,
            'whatsapp_instance' => $this->whatsapp_instance,
            'whatsapp_connected_at' => $this->whatsapp_connected_at,
            'email' => $this->email,
            'web' => $this->web,
            'logo_path' => $this->logo_path,
            'address' => $this->address,
            'city' => $this->city,
            'has_premises' => $this->has_premises,
            'description' => $this->description,
            'currency_id' => $this->currency_id,
            'reference_currency_id' => $this->reference_currency_id,
            'tax_condition_id' => $this->tax_condition_id,
            'tax_id' => $this->tax_id,
            'is_active' => $this->is_active,
            'sector' => $this->sector,
            'activity' => $this->activity,
        ];
    }

    public static function fromArray(array $data): self
    {
        return new self(
            name: $data['name'] ?? '',
            country_id: DtoCast::toNullableId($data['country_id'] ?? null),
            province_id: DtoCast::toNullableId($data['province_id'] ?? null),
            timezone: DtoCast::toNullableString($data['timezone'] ?? null),
            billing_email: $data['billing_email'] ?? '',
            whatsapp_number: DtoCast::toNullableString($data['whatsapp_number'] ?? null),
            fallback_whatsapp_number: DtoCast::toNullableString($data['fallback_whatsapp_number'] ?? null),
            whatsapp_instance: DtoCast::toNullableString($data['whatsapp_instance'] ?? null),
            whatsapp_connected_at: DtoCast::toNullableString($data['whatsapp_connected_at'] ?? null),
            email: DtoCast::toNullableString($data['email'] ?? null),
            web: DtoCast::toNullableString($data['web'] ?? null),
            logo_path: DtoCast::toNullableString($data['logo_path'] ?? null),
            address: DtoCast::toNullableString($data['address'] ?? null),
            city: DtoCast::toNullableString($data['city'] ?? null),
            has_premises: isset($data['has_premises']) ? (bool) $data['has_premises'] : null,
            description: DtoCast::toNullableString($data['description'] ?? null),
            currency_id: DtoCast::toNullableId($data['currency_id'] ?? null),
            reference_currency_id: DtoCast::toNullableId($data['reference_currency_id'] ?? null),
            tax_condition_id: DtoCast::toNullableId($data['tax_condition_id'] ?? null),
            tax_id: DtoCast::toNullableString($data['tax_id'] ?? null),
            is_active: $data['is_active'] ?? true,
            sector: DtoCast::toNullableString($data['sector'] ?? null),
            activity: DtoCast::toNullableString($data['activity'] ?? null),
        );
    }

    /**
     * Columns only: the sector and the activity stay out.
     *
     * Text is trimmed and nullable columns go back to null, so an empty one never
     * holds `''` — present but empty, which `whereNull` never finds.
     */
    public function toPayload(): array
    {
        return [
            'name' => DtoCast::squish($this->name) ?? '',
            'country_id' => $this->country_id,
            'province_id' => $this->province_id,
            'timezone' => DtoCast::squish($this->timezone),
            'billing_email' => DtoCast::squish($this->billing_email) ?? '',
            'whatsapp_number' => DtoCast::squish($this->whatsapp_number),
            'fallback_whatsapp_number' => DtoCast::squish($this->fallback_whatsapp_number),
            'whatsapp_instance' => DtoCast::toNullableString($this->whatsapp_instance),
            'whatsapp_connected_at' => DtoCast::toNullableString($this->whatsapp_connected_at),
            'handoff_level' => $this->handoff_level,
            'handoff_rules' => DtoCast::toNullableString(trim((string) $this->handoff_rules)),
            'email' => DtoCast::squish($this->email),
            'web' => DtoCast::squish($this->web),
            'logo_path' => DtoCast::toNullableString($this->logo_path),
            'address' => DtoCast::squish($this->address),
            'city' => DtoCast::squish($this->city),
            'has_premises' => $this->has_premises,
            // Trim only: squish would flatten the textarea's line breaks.
            'description' => DtoCast::toNullableString(trim((string) $this->description)),
            'currency_id' => $this->currency_id,
            'reference_currency_id' => $this->reference_currency_id,
            'tax_condition_id' => $this->tax_condition_id,
            'tax_id' => DtoCast::squish($this->tax_id),
            'is_active' => $this->is_active,
        ];
    }
}
